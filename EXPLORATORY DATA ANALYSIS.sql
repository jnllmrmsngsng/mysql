...
(SELECT SUBSTRING(`date`, 1,7) as `MONTH`, SUM(total_laid_off) AS total_off FROM layoffs_staging2
 ...
 FROM layoffs_staging2 GROUP BY company, YEAR(`date`) ORDER BY 3 DESC;
 ...
 , 
 Company_Year_Rank AS (SELECT *, DENSE_RANK() OVER (PARTITION BY years ORDER BY total_laid_off DESC) AS Ranking FROM Company_Year
